...
create unique index if not exists outreach_messages_campaign_lead_step_idx
  on public.outreach_messages(campaign_id, lead_id, step);

create index if not exists outreach_campaigns_user_id_created_at_idx
  on public.outreach_campaigns(user_id, created_at desc);

create index if not exists outreach_leads_campaign_id_idx
