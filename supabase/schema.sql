...
create index if not exists email_connections_user_id_created_at_idx on public.email_connections(user_id, created_at desc);
create unique index if not exists email_connections_user_provider_email_idx on public.email_connections(user_id, provider, lower(email));
...
create index if not exists outreach_campaigns_email_connection_id_idx on public.outreach_campaigns(email_connection_id);
